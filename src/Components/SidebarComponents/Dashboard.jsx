  ...
  return (
    <div className="md:h-auto md:pb-0 h-screen pb-1 flex flex-col overflow-hidden">
      ...
      <div className="flex-grow relative overflow-y-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 p-6 bg-gray-100 min-h-full">
          {/* Expiring Tasks Notification - Top Right Corner, Left of Expiring Services */}
          {isExpiringTasksPopupOpen && expiringTasks.length > 0 && (
            <div className="fixed top-4 right-120 bg-yellow-100 border-l-4 border-yellow-500 p-4 mb-4 w-64 md:w-80 lg:w-96 shadow-lg z-10">
              <div className="flex justify-between items-center mb-2">
                <h3 className="font-semibold text-red-500">Expiring Tasks</h3>
                <button
              ...
              <div className="max-h-64 overflow-y-auto">
                {expiringTasks.map((task) => (
                  <div
                    key={task._id}
                    className="bg-white p-3 rounded mb-2 hover:bg-yellow-50"
                  >
          ...
          {/* Expiring Services Notification - Top Right Corner with Increased Width */}
          {notifications.length > 0 && (
            <div className="fixed top-4 right-4 bg-yellow-100 border-l-4 border-yellow-500 p-4 mb-4 w-64 md:w-80 lg:w-96 shadow-lg z-10">
              <div className="flex justify-between items-center mb-2">
                <h3 className="font-semibold">Expiring Services</h3>
                <button
                  className="bg-red-500 text-white px-2 py-1 rounded text-sm"
              ...
              <div className="max-h-64 overflow-y-auto">
                {notifications.map((notification, index) => (
                  <p
                    key={index}
                    className="cursor-pointer text-sm mb-1 hover:bg-yellow-200 p-1 rounded"
