...
export default function CustomerComplaintsDashboard() {
  ...
  const handleUpdate = () => {
    setLoading(true);
    const finalStatus = useCustomStatus ? customStatus : updatedComplaint.status;
    
    fetch(
      `https://crm-mu-sooty.vercel.app/api/complaints/${selectedComplaint._id}`,
      {
        method: "PUT",
  ...
  const filteredComplaints = complaints.filter(complaint => {
    ...
    const matchesStatus = 
      statusFilter === "all" || 
      (statusFilter === "pending" && (!complaint.status || complaint.status === "Pending")) ||
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          {/* Complaints List */}
          <div className={`${selectedComplaint ? "lg:w-2/5" : "w-full"} bg-white rounded-lg shadow overflow-hidden`}>
                ...
                {filteredComplaints.map((complaint) => (
                  <li 
                    ...
                    className={`px-4 py-4 hover:bg-gray-50 cursor-pointer ${
                      selectedComplaint?._id === complaint._id ? "bg-indigo-50" : ""
                    }`}
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        <div className={`flex-shrink-0 h-8 w-8 rounded-full flex items-center justify-center text-white ${
                          !complaint.status || complaint.status === "Pending" ? "bg-red-500" :
                        ...
                        <div>
                          <p className="text-sm font-medium text-indigo-600 truncate">{complaint.subject}</p>
                          <p className="text-sm text-gray-500">{complaint.fullName}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-xs text-gray-500">{formatDate(complaint.createdAt)}</p>
                        <p className="text-xs font-medium text-gray-900">{complaint.projectName}</p>
                      </div>
                    </div>
                  </li>
                ))}
                ...
                {filteredComplaints.map((complaint) => (
                  <div 
                    ...
                    className={`border rounded-lg p-4 hover:shadow-md cursor-pointer ${
                      selectedComplaint?._id === complaint._id ? "ring-2 ring-indigo-500" : ""
                    }`}
                    onClick={() => handleViewComplaint(complaint)}
                  >
                    <div className="flex justify-between items-start">
                      ...
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        !complaint.status || complaint.status === "Pending" ? "bg-red-100 text-red-800" :
                        complaint.status === "In Progress" ? "bg-yellow-100 text-yellow-800" :
                        "bg-green-100 text-green-800"
                      }`}>
                        {complaint.customStatus || complaint.status || "Pending"}
                      </span>
                    </div>
                    <p className="mt-1 text-sm text-gray-500 line-clamp-2">{complaint.complaintDescription}</p>
                    <div className="mt-2 flex items-center justify-between">
                      <p className="text-xs text-gray-500">{complaint.fullName}</p>
                      <p className="text-xs text-gray-500">{formatDate(complaint.createdAt)}</p>
                    </div>
                  </div>
                ))}
              ...
              <div className="px-4 py-5 sm:p-6">
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-900">{selectedComplaint.complaintID}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-900">{selectedComplaint.fullName}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-900">{selectedComplaint.projectName}</p>
                  </div>
                  <div>
                    <h4 className="text-sm font-medium text-gray-500">Preferred Contact</h4>
                    <p className="mt-1 text-sm text-gray-900">{selectedComplaint.preferredContact}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    {selectedComplaint.attachment ? (
                      ...
                      <p className="mt-1 text-sm text-gray-500">No attachment</p>
                    )}
                  </div>
                </div>
                ...
                <div className="mt-6">
                  ...
                  <div className="mt-1 p-3 bg-gray-50 rounded-md">
                    <p className="text-sm text-gray-900 whitespace-pre-line">{selectedComplaint.complaintDescription}</p>
                  </div>
                </div>
                ...
                <div className="mt-6">
                  ...
                  <div className="mt-2 flex items-center">
                    ...
                    />
                    <label htmlFor="use-custom-status" className="ml-2 block text-sm text-gray-700">
                      Use custom status
                    </label>
                  </div>
      ...
      {showFeedbacks && (
        <div className="fixed inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
          <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div className="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true" onClick={() => setShowFeedbacks(false)}></div>
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                    ...
                    <div className="mt-2">
                      {feedbackLoading ? (
                        ...
                        <p className="text-gray-500 py-4">No feedback available</p>
                      ) : (
                                ...
                                <div className="ml-4 flex-1">
                                  <div className="flex items-center justify-between">
                                    ...
                                    <span className="text-xs text-gray-500">
                                      {feedback.timestamp ? formatDate(feedback.timestamp.toDate()) : "N/A"}
                                    </span>
                                  </div>
                                  <p className="text-xs text-gray-500 mt-1">{feedback.email}</p>
                                  <p className="mt-2 text-sm text-gray-700 whitespace-pre-line">{feedback.message}</p>
                                </div>
                              </div>
                            </div>
                          ))}
